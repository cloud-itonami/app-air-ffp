(ns air-ffp.worker
  ...
  ここには判断を置かない —— どのハンドラが答えるかは `air-ffp.route/dispatch`
  が決め、ページの中身は `air-ffp.view` が組む。どちらも `.cljc` なので、
  ブラウザもビルドも無しにテストできる。

  wrangler.jsonc の `main` は `dist/worker.js` を指し、それはこの名前空間を
  コンパイルしたものである。移行前は `svelte/.svelte-kit/cloudflare/_worker.js`
  という **この tree に存在しないパス**を指しており、読み手が開く `src/app.ts`
  ...
  (:require [air-ffp.route :as route]
...
(defn- env->map
  "env の **キーだけ** を keyword で拾い、値も持つ map を返す。

  この map から**ページへ出るのは キー名と、明示した 2 つの値だけ**
  （APP_CAPABILITIES と中継先 URL）。他の値は `view/body` に渡らない。"
  [env]
  (if env
  ...
  (let [e (env->map env)]
    (->response
     (view/render {:css dds-css
                   :routes route/routes
                   :vars (sort (keys e))
                   :capabilities (route/declared-capabilities (:APP_CAPABILITIES e))
...
(defn fetch-handler [req env _ctx]
  (let [url (js/URL. (.-url req))
    ...
    (case action
      ...
      :health (json {:ok true :app "air-ffp" :runtime "cljs"
